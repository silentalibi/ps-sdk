 ...
 *
 *
 *\section Photoshop251 Photoshop 25.1 - 2024
 *
 * Windows was built on a Windows 10 64 bit machine
 * using the 64 bit tool chain and the following version:
 * Microsoft Visual Studio 2022 v143
 * Version 17.0.5
 * Windows 10 SDK version 10.0.22621.0
 *
 * OS X was built on an OS X 12.6.5 machine
 * Using Xcode 14.2 (14C18), Base SDK 12.1, OS X Deployment Target of 11.0
 *
 *
 *\section Photoshop241 Photoshop 24.1 - 2023
 *
 * Windows was built on a Windows 10 64 bit machine
 * using the 64 bit tool chain and the following version:
 * Microsoft Visual Studio 2022 v143
 ...
 *
 * OS X was built on an OS X 11.6.5 machine
 * Using Xcode 13.2.1 (13C110), Base SDK 12.1, OS X Deployment Target of 11.0
 *
 *
 *\section Photoshop234 Photoshop 23.4 - 2022
 *
 * Windows was built on a Windows 10 64 bit machine
 * using the 64 bit tool chain and the following version:
 ...
 *
 * OS X was built on an OS X 10.15.7 machine
 * Using Xcode 12.4 (12D4e), Base SDK 11.0, OS X Deployment Target of 10.15
 *
 *
 *\section Photoshop223 Photoshop 22.3 - 2021
 *
 * Windows was built on a Windows 10 64 bit machine
 * using the 64 bit tool chain and the following version:
 * Microsoft Visual Studio 2019 v142
 ...
 *
 * OS X was built on an OS X 10.15.7 machine
 * Using Xcode 12.2 (12B45b), Base SDK 11.0, OS X Deployment Target of 10.14
 *
 *
 *\section Photoshop22 Photoshop 22.0 - 2021
 *
 * Windows was built on a Windows 10 64 bit machine
 * using the 64 bit tool chain and the following version:
 * Microsoft Visual Studio 2019 v142
 ...
 * Windows 10 SDK version 10.0.18362.0
 *
 * OS X was built on an OS X 10.14.4 machine
 * Using Xcode 11.3 (11C29), Base SDK 10.15, OS X Deployment Target of 10.14
 *
 *
 * \section Photoshop21 Photoshop 21.0 - 2020
 *
 * Windows was built on a Windows 10 64 bit machine
 * using the 64 bit tool chain and the following version:
 ...
 *
 * OS X was built on an OS X 10.14.4 machine
 * Using Xcode 10.2.1 (10E1001), Base SDK 10.14, OS X Deployment Target of 10.13
 *
 *
...
* If a plug-in needs to examine the elements of another plug-ins PiPL for some reason or 
...
* Code descriptors tell Photoshop the type and location of a plug-ins code. More 
...
// 'mi64' <PIMacCodeDesc> Intel 64 Mach-O code descriptor. See struct below.
#define PICodeMacIntel64Property	'mi64'

// 'ma64' <PIMacCodeDesc> ARM 64 Mach-O code descriptor. See struct below.
#define PICodeMacARM64Property	'ma64'

///@ingroup PiPLCodeDescriptorKeys

// '8664' <PIWin64CodeDesc> Win32 Intel code descriptor. See struct below.
#define PIWin64X86CodeProperty  0x38363634L

// 'wa86' <PIWin64CodeDesc> Win32 ARM code descriptor. See struct below.
#define PIWin64ARMCodeProperty  0x77613634L
...
// For 'ma64' PICodeMacARM64Property:
typedef struct PIMacCodeDesc
{
...
// For compatibilty with previous name for PIMacCodeDesc
typedef PIMacCodeDesc PIMacIntelCodeDesc;
...
typedef struct PIWin64CodeDesc
{
	char fEntryName[1];
} PIWin64CodeDesc;

// For compatibility with previous name of PIWin64CodeDesc
typedef PIWin64CodeDesc PIWin64X86CodeDesc;
...
* in Photoshops menus based on the current documents image mode.
*/
...
{
	Fixed gamma;	  /**< The monitors gamma value or zero if the whole record is invalid. */
	Fixed redX;		  /**< The chromaticity coordinates of the monitors phosphors */
	Fixed redY;		  /**< The chromaticity coordinates of the monitors phosphors */
	Fixed greenX;	  /**< The chromaticity coordinates of the monitors phosphors */
	Fixed greenY;	  /**< The chromaticity coordinates of the monitors phosphors */
	Fixed blueX;	  /**< The chromaticity coordinates of the monitors phosphors */
	Fixed blueY;	  /**< The chromaticity coordinates of the monitors phosphors */
	Fixed whiteX;	  /**< The chromaticity coordinates of the monitors white point. */
	Fixed whiteY;	  /**< The chromaticity coordinates of the monitors white point. */
...
typedef struct PSPixelMap
{
	...
	PSPixelMask *mat;	/**< A pixel mask to use for matting correction. Can be specified in 
						...
						* to remove the white fringe. This field points to a @ref PSPixelMask structure 
						* with a maskDescription that indicates what type of matting needs 
	...
	PSPixelMask *masks;	/**< A pointer to a chain of \c PSPixelMasks that are multiplied together 
						...
						transparent and should have the checkerboard displayed. @ref kSimplePSMask, @ref kBlackMatPSMask, 
						@ref kWhiteMatPSMask, and @ref kGrayMatPSMask all operate such that 255=opaque and 0=transparent. 
						@ref kInvertPSMask has 255=transparent and 0 = opaque. 
						Depth of data in the mask must match the depth in the color channels */
	
	// Use to set the phase of the checkerboard:
	int32 maskPhaseRow; /**< The phase of the checkerboard with respect to the top left corner of the @ref PSPixelMap */
	int32 maskPhaseCol; /**< The phase of the checkerboard with respect to the top left corner of the @ref PSPixelMap */
...
*/
	...
	* verify the host signature (in the parameter blocks \c hostSig field) before 
	...
	* Each repeated call must go through your plug-ins \c main() entry point. With \c AdvanceStateProc, a plug-in 
	...
	* -	choose a color using the users preferred color picker 
...
*/
	...
	* down for the plug-ins benefit, such as the continue calls to Filter and Export plug-ins.
	*/
...
*/
	...
	* coordinates do not "fall through the cracks." 
	...
	* switching between point sampling and linear interpolation. If Photoshop did not do this, 
	* a plug-in could end up modifying pixels using point sampling that would not get modified 
...
*/
	...
	* the bounds of the port, they will not be written.
